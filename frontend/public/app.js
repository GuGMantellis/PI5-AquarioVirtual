...
const phBadgeValEl = document.getElementById('phBadgeVal');
...
function setupFirebaseListeners() {
  ...
  // Monitorar pH atual
  database.ref("/aquarium/ph").on("value", (snapshot) => {
    const val = snapshot.val();
    if (val !== null) {
    ...
    console.error("Erro ao ler pH do Firebase: ", error);
  });
...
function startSimulationMode() {
  ...
  window.simInterval = setInterval(() => {
    ...
    // Simula flutuação do pH
    const phDelta = (Math.random() - 0.5) * 0.12;
    currentPh = parseFloat((currentPh + phDelta).toFixed(2));
    if (currentPh < 5.5) currentPh = 5.6;
    if (currentPh > 9.0) currentPh = 8.9;
...
function updatePhUI() {
  if (!currentPhEl) return;
  currentPhEl.textContent = currentPh.toFixed(1);
  if (phBadgeValEl) phBadgeValEl.textContent = currentPh.toFixed(1);

  phStatusEl.className = "info-value status-tag";

  if (currentPh < 6.0) {
    phStatusEl.classList.add('status-hot');
    phStatusEl.textContent = "Muito Ácido!";
  } else if (currentPh < 6.5) {
    phStatusEl.classList.add('status-cold');
    phStatusEl.textContent = "Ácido";
  } else if (currentPh <= 8.0) {
    phStatusEl.classList.add('status-safe');
    phStatusEl.textContent = "Neutro ✔";
  } else if (currentPh <= 8.5) {
    phStatusEl.classList.add('status-cold');
    phStatusEl.textContent = "Alcalino";
  } else {
    phStatusEl.classList.add('status-hot');
    phStatusEl.textContent = "Muito Alcalino!";
  }
}
